vu.help = {
	sections: {
        ...
        play: "Walk around the world on the explore page, and hang out with the other players. Move your avatar with WASD, rotate with Q and E, press SPACE to jump, and hold SHIFT to run. Press ENTER to walk through a door, and jump out of fire to fly. Click any person or most things to pull up an interactive menu - use this to chat with an automaton, read a book, etc. Switch between cameras with the Camera menu at the top of the screen (most players prefer to navigate with the polar, behind, or pov cams), and use the arrow keys (along with the mouse and mouse wheel [two finger drag]) for finer-grained camera control. The basic (test page) menus are at the bottom of the page: Triggers and Vibes, bottom left; Gestures, Dances, and Mods, bottom right. Also included are the Map at the top right, the Chat menu at the bottom and the Run Home button at the top (only visible when you're away from your home room). If you have any questions, click the 'help!' button or say 'enable help mode' while chatting with any automaton. Anything you say or do is relayed to anyone else nearby, so play responsibly!",
